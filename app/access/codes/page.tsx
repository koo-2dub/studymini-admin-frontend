  ...
  return (
    <>
      <PageHeader
        ...
        title="수강코드 관리"
        description="B2B와 체험단 수강코드 생성, 배포, 사용 현황을 통합 관리합니다. 모든 코드는 개인별 코드이며 1인 1회 사용으로 고정됩니다."
      ...
      <Card>
        <CardHeader className="gap-4 lg:flex-row lg:items-start lg:justify-between lg:space-y-0">
          <div className="space-y-2">
            <CardTitle>수강코드 목록</CardTitle>
            <CardDescription>B2B/체험단 구분, 대상 콘텐츠, 발급 수량과 사용률을 확인합니다.</CardDescription>
          </div>
              ...
              {accessCodeCampaigns.map((campaign) => {
                ...
                return (
                  <TableRow key={campaign.id} className="cursor-pointer">
                    <TableCell>
                      <Link href={detailHref} className="block min-w-[260px] py-1">
                    ...
                    <TableCell className="whitespace-nowrap font-semibold text-slate-700">
                      <Link href={detailHref} className="block py-1"><Badge variant={categoryVariant(campaign.category)}>{campaign.category}</Badge></Link>
                    </TableCell>
