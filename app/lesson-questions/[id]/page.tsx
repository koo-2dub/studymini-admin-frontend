...
export default async function LearningInquiryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const inquiry = learningInquiries.find((item) => item.id === id);

  if (!inquiry) notFound();

  return (
    <div className="space-y-6">
      <PageHeader eyebrow="학습 문의 상세" title={inquiry.title} description="목록과 분리된 상세 페이지에서 학습 문의 내용과 답변 처리 정보를 확인합니다." />
      <Button asChild variant="outline"><Link href="/lesson-questions">학습 문의 목록으로</Link></Button>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <DetailItem label="언어" value={inquiry.language} />
          <DetailItem label="강의" value={inquiry.course} />
          <DetailItem label="레슨" value={inquiry.lesson} />
          <DetailItem label="공개상태" value={<Badge variant={inquiry.visibility === "공개" ? "success" : "slate"}>{inquiry.visibility}</Badge>} />
